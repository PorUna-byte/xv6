...
e1000_transmit(struct mbuf *m)
{
  ...
  //we reserve a descriptor slot to distinguish full from empty
  ...
  if((tail+1)%TX_RING_SIZE==head || !(tx_ring[tail].status & E1000_TXD_STAT_DD)){
    release(&e1000_lock);
    return -1;
  }
  if(tx_mbufs[tail])
